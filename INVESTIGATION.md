...
### [91] decoder_threads regression: 6→0 during unified decoder removal refactor
**Date:** 2026-03-16
**Idea:** Performance investigation started with establishing a baseline for the v2 format. Discovered that `decoder_threads` had been accidentally set to 0 during the refactor that removed the unified decoder.
**Change:** Identified the regression in `create_decoder_session` — `decoder_threads` was 0 instead of 6. Fixed to restore the correct thread count.
**Result:**
| Before fix | After fix |
|---|---|
| ~3.97s | ~1.98s |

**Outcome:** IMPROVED — restoring thread count halved inference time. The bug was introduced silently during code restructuring; no test caught it because correctness was unaffected.
**Notes:** The v1 1.44s benchmark from earlier sessions was measured with correct thread count. The v2 baseline of ~2.01s is measured after this fix.

### [92] v2 format per-step timing and decoder working-set analysis
**Date:** 2026-03-16
**Idea:** The v2 AWQ INT8 format (embedding table in ONNX graph) measured ~2.01s vs the v1 reference of ~1.44s. Investigated whether the gap came from Gather+DequantizeLinear overhead or from the increased decoder working-set size causing L3 cache pressure. Tested: (a) MatMul-only INT8 leaving embed FP32, (b) weight re-tying to share embed_tokens and lm_head as a single quantized tensor, eliminating the 156 MB duplicate.
**Change:** Added timing instrumentation to model.rs for per-step latency. Ran three decoder variants: baseline, MatMul-only, and weight-retied (using `embed_tokens.weight` for both Gather and lm_head, with AWQ scale applied separately).
**Result:**
| Experiment | Per-step | Decoder weights |
|---|---|---|
| v2 AWQ INT8 untied (baseline) | 35.5ms | 717 MB |
| MatMul-only INT8, FP32 embed | 40–47ms | 1218 MB |
| Tied-weight INT8 shared tensor | 83–90ms | 598 MB |
| v1 reference | ~26ms | 596 MB |

Prefill: 320ms. Per-step × 29 steps ≈ 1030ms. Total ≈ 1350ms.

**Outcome:** NO IMPROVEMENT — all alternatives are slower than the untied baseline.
- MatMul-only: FP32 embed table (622 MB) increases L3 cache pressure more than the Gather+Dequant cost it eliminates.
- Tied-weight: Gather needs `[vocab, hidden]` layout, MatMulInteger needs `[hidden, vocab]`. ORT inserts a 156 MB Transpose per step; DequantizeLinear nodes prevent constant-folding. 83ms/step vs 35.5ms — 2.3× slower.
- The 35.5ms vs 26ms gap (37%) is attributed to the AWQ-untied 156 MB duplicate embedding weight in the 717 MB session increasing overall working set vs v1's 596 MB.

All weight-sharing approaches reverted. v2 accepted at 2.01s.
**Notes:** AWQ smoothing is essential for INT8 speed: naive INT8 without AWQ ran at ~125ms/step (3.5× slower) because unsmoothed weights produce large per-tensor scale errors.

### [93] v3 hybrid decoder format: self-contained decoder_step with input_embeds
**Date:** 2026-03-16
**Idea:** In v1, Rust pre-loaded `embed_tokens.bin` as a contiguous FP32 lookup table; each step was a single zero-ORT-overhead ndarray row copy (~4 KB). The key insight: decoder_step processes one token and needs only a trivial embedding lookup — it does not need the in-graph Gather. Only decoder_init needs the embedding in-graph for the audio scatter across the full prompt sequence.

Hybrid approach:
- `decoder_init.onnx`: keeps `input_ids` + in-graph Gather + audio scatter (v2 interface)
- `decoder_step.onnx`: `input_embeds` (no embedding table in graph), self-contained (~571 MB INT8, under the 2 GB protobuf limit)
- Export pipeline writes `embed_tokens.bin` alongside ONNX files
- `decoder_init.onnx` uses its own `decoder_init.onnx.data` (standard per-model external data)
- Rust loads `embed_tokens.bin` at startup, performs row-lookup per step

**Change:** Updated `decoder_wrapper.py` `DecoderStepWrapper` to accept `input_embeds`. Export pipeline saves `embed_tokens.bin`. Quantizer inlines decoder_step weights (no shared `decoder_weights.data`). Rust `Qwen3Model` loads `embed_tokens.bin`, performs lookup, passes `input_embeds` to decoder_step.
**Result:**
| Model | Mean (11s JFK) | RTF | decoder_step |
|---|---|---|---|
| v2 AWQ INT8 | 2.01s | 0.18x | shared decoder_weights.data, 717 MB |
| v3 AWQ INT8 (self-contained step) | 2.01s | 0.18x | 571 MB self-contained |
| WER (200 samples) | — | — | 5.18% |

v3 decoder_step: 2970 nodes, 751 initializers, 596 MB — structurally identical to v1.

**Outcome:** NEUTRAL on performance; format improvement. Speed matches v2 exactly. The bottleneck is the transformer layer compute, not the embedding table overhead. WER 5.18% is within noise of previous 5.21% — no quality regression. The v3 format is standard (per-decoder external data, self-contained step file under 2 GB).
**Notes:** An intermediate attempt where decoder_step still used the shared `decoder_weights.data` measured 2.11s — ORT memory-maps the full shared file even when the step graph only references a subset. Making decoder_step self-contained resolved that overhead and matched v2 exactly. Committed as the final format.

### [94] v1 1.44s baseline reproducibility check
**Date:** 2026-03-16
**Idea:** The 1.44s benchmark from earlier sessions had never been reproduced. All v2/v3 measurements showed ~2.01–2.07s. Built the exact v1 code (from backup branch `feat/qwen3-batch-dev`) in an isolated worktree and benchmarked v1 code + v1 model back-to-back with v3 code + v3 model under identical conditions.
**Change:** Temporary worktree from backup branch tip. Built `--release`. Ran `bench_compare` on both sequentially.
**Result:**
| Code | Model | Mean (11s JFK) | RTF |
|---|---|---|---|
| v1 (feat/qwen3-batch-dev tip) | v1 INT8 trial-alpha-020 | 2.07s | 0.19x |
| v3 (current HEAD) | v3 INT8 AWQ α=0.2 | 2.07s | 0.19x |

**Outcome:** FINDING — no regression. v1 code + v1 model gives identical 2.07s as v3 code + v3 model. The 1.44s was a non-reproducible transient measurement — likely from lower system load or warm ORT caches in the original session. The v1 and v3 decoder_step ONNX graphs are structurally identical (2970 nodes, 751 initializers, 596 MB). There is no format-induced regression. The 1.44s figure should not be used as a performance target.
**Notes:** Temporary worktree deleted after comparison.

### [95] 1.7B model v3 exports: FP32, FP16, dynamic INT8
**Date:** 2026-03-16 to 2026-03-17
**Idea:** All existing 1.7B model directories were in v1 format (`input_embeds` for both decoders). Re-export in v3 hybrid format to align with the unified standard.
**Change:** Re-exported using updated `export.py` (v3 hybrid wrappers). FP16 required ORT FP16 optimizer applied to external-data ONNX before inlining (FP32 decoder_step at 6.5 GB exceeds 2 GB protobuf limit so stays external). FP16 export OOM-crashed on first attempt (peak ~20 GB for ORT optimizer on 6.5 GB FP32 decoder); succeeded by converting each file separately in sequence on remote host `step` (47 GB RAM).
**Result:**
| Variant | Disk | Test result |
|---|---|---|
| 1.7B FP32 v3 | 16 GB | 402s, passes |
| 1.7B FP16 v3 | 8.2 GB | Skipped on CPU (too slow); GPU target |
| 1.7B INT8 dynamic (no AWQ) | 4.7 GB | 26s, passes |

**Outcome:** NEUTRAL (format standardization). All 1.7B variants now use v3 hybrid interface. INT8 dynamic is a temporary stand-in pending GPTQ int4 calibration for v3 format.
**Notes:** INT8 dynamic superseded by int4 variants in [96]–[98].

### [96] 1.7B int4 RTN accuracy_level=4 (v3 format)
**Date:** 2026-03-17
**Idea:** GPTQ calibration data needed regenerating for v3 format. RTN al4 requires no calibration and was a component of the v1 winning combo. Applied to both decoders as a fast baseline.
**Change:** `quantize_nbits.py --bits 4 --block-size 64 --accuracy-level 4 --algo rtn` on both 1.7B v3 decoders. Added `Quantization::Int4` variant to Rust `resolve_model_path`.
**Result:**
| Metric | Value |
|---|---|
| Model size | 5.2 GB |
| Rust integration test | 21.5s, passes |
| WER (200 samples, `step`) | 4.25% |
| RTF (11s JFK) | 0.37x |
| Load time | 26.6s |

**Outcome:** IMPROVED over INT8 dynamic (26s → 21.5s test time, 4.25% WER). Adopted as the 1.7B quantized variant pending GPTQ comparison.

### [97] 1.7B GPTQ calibration on remote host `step` — sequential collection to avoid OOM
**Date:** 2026-03-17
**Idea:** GPTQ requires real decoder inputs captured via full inference. v1 calibration data was incompatible with v3 format. Running both decoder_init and decoder_step collection concurrently OOM-killed both jobs.
**Change:** Synced export pipeline to `step` (24 cores, 47 GB RAM). Updated `collect_gptq_calib.py` for v3 I/O schema. Installed `torchcodec` for HuggingFace audio streaming. Ran sequentially: decoder_init first, then decoder_step alone.
**Result:**
| Job | Samples | Output size | Peak RSS |
|---|---|---|---|
| decoder_init calibration | 32 | 18 MB | ~5 GB |
| decoder_step calibration | 256 (32×8 steps) | 4.98 GB | ~17 GB |

Both completed successfully when run sequentially. Concurrent execution OOM-killed both (KV cache accumulation: 28 layers × seq_len × hidden_dim × 32 samples × 8 steps).

**Outcome:** FINDING — decoder_step calibration must always run sequentially after decoder_init on memory-constrained hosts. The 24-core `step` host saturated all cores during GPTQ quantization (1700–2000% CPU observed).
**Notes:** `PyGILState_Release` crash at Python exit was a torch cleanup artifact — harmless, data was saved before exit. `src/` Python module was missing from initial rsync; fixed by resyncing separately.

### [98] 1.7B GPTQ-init + RTN al4 decoder_step: identical WER to RTN-only, faster load time
**Date:** 2026-03-17
**Idea:** From v1 experiment [89], GPTQ-init + RTN-step gave 4.25% WER vs RTN-only 4.33% — a measurable improvement. With v3 calibration data available, tested whether GPTQ still provides a WER advantage.
**Change:** GPTQ quantization on decoder_init using `1.7b_v3_gptq_init.npz` calibration. RTN al4 for decoder_step. Assembled hybrid. 200-sample WER evaluation on `step` for both RTN-only and GPTQ-init variants in the same session.
**Result:**
| Model | WER (200 samples) | RTF (11s) | Load time | Size |
|---|---|---|---|---|
| RTN al4 (both decoders) | 4.25% | 0.37x | 26.6s | 5.2 GB |
| GPTQ-init + RTN al4 step | 4.25% | 0.37x | 15.8s | 5.2 GB |

**Outcome:** FINDING — no WER difference. GPTQ provides no measurable quality improvement over RTN al4 for the 1.7B v3 model on 200 samples. The GPTQ variant loads 41% faster (15.8s vs 26.6s) with identical size and inference speed. GPTQ-init adopted as the production variant because the load time advantage is permanent and the calibration pipeline is now fully automated.
**Notes:** The v1 experiment [89] showing 4.33% → 4.25% improvement was likely due to different calibration conditions rather than a fundamental GPTQ advantage at this scale. The `quantize_nbits.py` script was updated to handle the GPTQ pipeline internally: PID-scoped config.json hiding (to prevent neural_compressor's `AutoConfig` from rejecting `model_type: qwen3_asr`), `quantization` config field as a dict keyed by decoder name (merged across split-decoder runs), and `SerializeToString()` replacing `onnx.save` for proto-only writes (the `onnx.save` after `load_external_data=False` would corrupt external data).

### [99] ONNX compliance: com.microsoft opset import for MatMulNBits; consolidated directory layout
**Date:** 2026-03-17
**Idea:** Audit all exported models against ONNX best practices before distribution. Consolidate per-variant directories into a single directory per model size with suffixed filenames so the `Quantization` enum in Rust can select files without changing directory.
**Change:**
1. ORT's `MatMulNBitsQuantizer` omits the `com.microsoft` opset import when inserting `MatMulNBits` nodes. Added post-quantization fixup in `quantize_nbits.py`. Patched all existing int4 models.
2. `quantize.py` (INT8 AWQ) updated to output `.int8.`-suffixed filenames (`decoder_init.int8.onnx`, etc.).
3. All 0.6B and 1.7B model variants consolidated into two directories. Separate quantization directories deleted (~10+ GB freed).

**Result:**
| Check | Status |
|---|---|
| `onnx.checker.check_model` | All 6 model variants PASS |
| `com.microsoft` opset import | Present in all int4 models |
| External data refs | `{name}.onnx.data` (standard naming) |
| Dynamic axes | Named (`batch`, `seq_len`, `past_seq_len`) |
| IR version / opset | 10 / ai.onnx v18 |

Final layout:
```
qwen3-asr-0.6b/    ~10 GB   (encoder.onnx, *.int4.onnx, *.int8.onnx, *.fp16.onnx,
                              embed_tokens.bin, config.json, tokenizer.json)
qwen3-asr-1.7b/    ~26 GB   (same pattern)
```

**Outcome:** IMPROVED — all models pass strict ONNX validation. Unified directory layout aligns with the existing `resolve_model_path` suffix convention used by Parakeet and other engines. Distribution tarballs can be per-variant while sharing common files.
**Notes:** The `onnx.save` corruption bug was latent — ORT tolerated it at load time. FP32 models (no `MatMulNBits`) pass ONNX validation without the opset fix. The old separate directories were deleted after consolidation. The `v3` suffix was dropped from directory names — `export.py` already outputs `qwen3-asr-{0.6b,1.7b}/` without version suffix.

### [100] 0.6B int4 RTN al4 export and encoder quantization comparison
**Date:** 2026-03-17
**Idea:** The 0.6B INT8 AWQ variant (5.21% WER) was previously the recommended 0.6B quantization. int4 RTN al4 measured 5.08% WER in earlier trials (experiment [87]) with FP32 encoder. Exported 0.6B int4 to the consolidated directory and tested which encoder quantization to pair with int4 decoders.
**Change:** `quantize_nbits.py --bits 4 --block-size 64 --accuracy-level 4` on 0.6B. Tested three encoder variants with int4 decoders: FP32 (717 MB), INT8 AWQ (197 MB), FP16 (359 MB). Created `encoder.int4.onnx` as a copy of `encoder.fp16.onnx`.
**Result:**
| Encoder | Encoder size | WER (200 samples, ORT 1.24) | Mean (11s JFK) | RTF |
|---|---|---|---|---|
| FP32 | 717 MB | 5.08% | 1.73s | 0.16x |
| INT8 AWQ | 197 MB | 6.01% | 1.68s | 0.15x |
| FP16 | 359 MB | 5.16% | ~1.68s | 0.15x |

Total int4 package sizes:
| Encoder | Total download |
|---|---|
| FP32 | ~2.5 GB |
| INT8 | ~1.9 GB |
| FP16 | ~2.1 GB |

**Outcome:** FINDING — INT8 encoder degrades 0.6B WER by ~1pp (6.01% vs 5.08%). The windowed Conv2D encoder is sensitive to INT8 dynamic quantization. FP16 encoder has no measurable WER impact vs FP32 (5.16% vs 5.08% is within the ±0.5pp confidence interval for 200 samples) at half the size. FP16 encoder is also faster than FP32 (1.68s vs 1.73s).
**Notes:** `encoder.int4.onnx` = FP16 encoder adopted as the default for int4 variants. On CPU, ORT promotes FP16 weights to FP32 for computation (via Cast nodes), so the only quality loss is from FP16 rounding in stored weights — negligible. The `resolve_model_path` int4→int8 fallback chain was tested and reverted: it would silently use the INT8 encoder for int4 models, causing the 1pp WER regression. Final fallback is simple: try `{name}.int4.onnx`, fall back to `{name}.onnx` (FP32).

### [101] 0.6B int4 with FP16 encoder as new recommended variant
**Date:** 2026-03-17
**Idea:** With int4 decoders + FP16 encoder at 5.16% WER / 0.15x RTF / 2.1 GB, this beats the previous INT8 AWQ recommendation (5.52% WER on ORT 1.23 `step` host / 5.21% on ORT 1.24 local / 0.19x RTF / 1.1 GB) on both WER and speed. The 1 GB size increase is acceptable.
**Change:** Updated README, memory, and Handy model recommendations. `encoder.int4.onnx` (FP16 copy) added to both 0.6B and 1.7B model directories. `Quantization::Int4` resolves all three files with `.int4` suffix.
**Result:**
| Model | Variant | WER | RTF | Download |
|---|---|---|---|---|
| **0.6B** | int4 (FP16 enc + RTN al4 dec) | 5.16% | 0.15x | 2.1 GB |
| **0.6B** | INT8 AWQ (previous rec) | 5.21–5.52% | 0.19x | 1.1 GB |
| **1.7B** | int4 (FP16 enc + GPTQ/RTN dec) | 4.25% | 0.37x | ~4.3 GB |
| Parakeet-TDT INT8 | — | 5.45% | 0.16x | — |

**Outcome:** IMPROVED — int4 is now the recommended variant for both model sizes. 0.6B int4 matches Parakeet speed (0.15x vs 0.16x RTF) with lower WER (5.16% vs 5.45%) and full punctuation output.
**Notes:** WER measurements vary by ~0.3pp between ORT versions (1.23 on `step` vs 1.24 local). The `step` host measured 5.48% for int4+FP16enc and 5.52% for INT8 AWQ — the relative ranking is consistent across ORT versions. Speed benchmarks should be compared within the same host/ORT version.
